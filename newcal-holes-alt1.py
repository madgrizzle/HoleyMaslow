...
adjustMotorTilt = False  # this allows tilting of top beam
...
while(errorMagnitude > acceptableTolerance and n < numberOfIterations):
	...
	if (holePattern == 3):
		LChainErrorHole5 = LChainLengthHole5Est - LChainLengthHole5 + errorLH1#
		LChainErrorHole6 = LChainLengthHole6Est - LChainLengthHole6 + errorLH2#
		LChainErrorHole7 = LChainLengthHole7Est - LChainLengthHole7 + errorLH3#
		LChainErrorHole8 = LChainLengthHole8Est - LChainLengthHole8 + errorLH4#
		RChainErrorHole5 = RChainLengthHole5Est - RChainLengthHole5 + errorRH1#
		RChainErrorHole6 = RChainLengthHole6Est - RChainLengthHole6 + errorRH2#
		RChainErrorHole7 = RChainLengthHole7Est - RChainLengthHole7 + errorRH3#
		RChainErrorHole8 = RChainLengthHole8Est - RChainLengthHole8 + errorRH4#
...
#		errorMagnitude = math.sqrt( (LChainErrorHole1*LChainErrorHole1 + LChainErrorHole2*LChainErrorHole2 + LChainErrorHole3*LChainErrorHole3 + LChainErrorHole4*LChainErrorHole4 +  LChainErrorHole5*LChainErrorHole5 + LChainErrorHole6*LChainErrorHole6 + LChainErrorHole7*LChainErrorHole7 + LChainErrorHole8*LChainErrorHole8 + RChainErrorHole1*RChainErrorHole1 + RChainErrorHole2*RChainErrorHole2 + RChainErrorHole3*RChainErrorHole3 + RChainErrorHole4*RChainErrorHole4 +  RChainErrorHole5*RChainErrorHole5 + RChainErrorHole6*RChainErrorHole6 + RChainErrorHole7*RChainErrorHole7 + RChainErrorHole8*RChainErrorHole8) / 16.0)
	...
	#find worse error:
	bestLChainErrorHole1 = 9999.9
	bestLChainErrorHole2 = 9999.9
	bestLChainErrorHole3 = 9999.9
	bestLChainErrorHole4 = 9999.9
	bestRChainErrorHole1 = 9999.9
	bestRChainErrorHole2 = 9999.9
	bestRChainErrorHole3 = 9999.9
	bestRChainErrorHole4 = 9999.9
	if (holePattern ==3):
		bestLChainErrorHole5 = 9999.9
		bestLChainErrorHole6 = 9999.9
		bestLChainErrorHole7 = 9999.9
		bestLChainErrorHole8 = 9999.9
		bestRChainErrorHole5 = 9999.9
		bestRChainErrorHole6 = 9999.9
		bestRChainErrorHole7 = 9999.9
		bestRChainErrorHole8 = 9999.9

	maxError = LChainErrorHole1
	errorImprovement = False
	if (LChainErrorHole1 < bestLChainErrorHole1):
		errorImprovement = True
	if (LChainErrorHole2 > maxError):
		maxError = LChainErrorHole2
		if (LChainErrorHole2 < bestLChainErrorHole2):
			errorImprovement = True
		else:
			errorImprovement = False
	if (LChainErrorHole3 > maxError):
		maxError = LChainErrorHole3
		if (LChainErrorHole3 < bestLChainErrorHole3):
			errorImprovement = True
		else:
			errorImprovement = False
	if (LChainErrorHole4 > maxError):
		maxError = LChainErrorHole4
		if (LChainErrorHole4 < bestLChainErrorHole4):
			errorImprovement = True
		else:
			errorImprovement = False
	if (RChainErrorHole1 > maxError):
		maxError = RChainErrorHole1
		if (RChainErrorHole1 < bestRChainErrorHole1):
			errorImprovement = True
		else:
			errorImprovement = False
	if (RChainErrorHole2 > maxError):
		maxError = RChainErrorHole2
		if (RChainErrorHole2 < bestRChainErrorHole2):
			errorImprovement = True
		else:
			errorImprovement = False
	if (RChainErrorHole3 > maxError):
		maxError = RChainErrorHole3
		if (RChainErrorHole3 < bestRChainErrorHole3):
			errorImprovement = True
		else:
			errorImprovement = False
	if (RChainErrorHole4 > maxError):
		maxError = RChainErrorHole4
		if (RChainErrorHole4 < bestRChainErrorHole4):
			errorImprovement = True
		else:
			errorImprovement = False
	if (holePattern == 3):
		if (LChainErrorHole5 > maxError):
			maxError = LChainErrorHole5
			if (LChainErrorHole1 < bestLChainErrorHole1):
				errorImprovement = True
			else:
				errorImprovement = False
		if (LChainErrorHole6 > maxError):
			maxError = LChainErrorHole6
			if (LChainErrorHole6 < bestLChainErrorHole6):
				errorImprovement = True
			else:
				errorImprovement = False
		if (LChainErrorHole7 > maxError):
			maxError = LChainErrorHole7
			if (LChainErrorHole7 < bestLChainErrorHole7):
				errorImprovement = True
			else:
				errorImprovement = False
		if (LChainErrorHole8 > maxError):
			maxError = LChainErrorHole8
			if (LChainErrorHole8 < bestLChainErrorHole8):
				errorImprovement = True
			else:
				errorImprovement = False
		if (RChainErrorHole5 > maxError):
			maxError = RChainErrorHole1
			if (RChainErrorHole5 < bestRChainErrorHole5):
				errorImprovement = True
			else:
				errorImprovement = False
		if (RChainErrorHole6 > maxError):
			maxError = RChainErrorHole2
			if (RChainErrorHole6 < bestRChainErrorHole6):
				errorImprovement = True
			else:
				errorImprovement = False
		if (RChainErrorHole7 > maxError):
			maxError = RChainErrorHole3
			if (RChainErrorHole7 < bestRChainErrorHole7):
				errorImprovement = True
			else:
				errorImprovement = False
		if (RChainErrorHole8 > maxError):
			maxError = RChainErrorHole8
			if (RChainErrorHole8 < bestRChainErrorHole8):
				errorImprovement = True
			else:
				errorImprovement = False

	if (errorImprovement == False):
